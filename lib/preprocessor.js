...
var preprocessor = {
  ...
  execute : function (template, options, callback) {
    ...
    // Scan image placeholders carrying `:generateImage()` markers and inject
    // the hidden text runs the builder will duplicate per loop iteration.
    image.scanImageMarkers(template, template.extension);
