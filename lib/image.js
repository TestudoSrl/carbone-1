...
// Two accepted marker shapes inside an image's descr / alt-text. Either form
// flags the placeholder as a dynamic row-level image, matching what Carbone
// Cloud / EE already do in practice:
//
//   {d.rows[i].x.signatureImage}                 (bare path — common)
//   {d.rows[i].x.signatureImage:generateImage()} (explicit formatter)
//
// A descr carrying any *other* formatter (e.g. `:formatD(...)`, `:ifEQ(...)`)
// is NOT recognized as an image marker: the builder would resolve it to a
// non-image string and parseDataUri would reject it. Those descriptors are
// left untouched.
var GENERATE_IMAGE_MARKER_RE_BARE = /^\s*\{\s*([cdt][.\[][^}:]+?)\s*\}\s*$/;
var GENERATE_IMAGE_MARKER_RE_FORMATTER =
  /^\s*\{\s*([cdt][.\[][^}]*?)\s*:\s*generateImage\s*\([^}]*\)\s*\}\s*$/;

function extractImageMarker (descr) {
  if (typeof descr !== 'string') { return null; }
  var m = GENERATE_IMAGE_MARKER_RE_BARE.exec(descr);
  if (m) { return { path : m[1].trim() }; }
  m = GENERATE_IMAGE_MARKER_RE_FORMATTER.exec(descr);
  if (m) { return { path : m[1].trim() }; }
  return null;
}
...
var image = {
  ...
  _internal : {
    ...
    extractImageMarker  : extractImageMarker
  }
...
function scanDocxContent (xml, relsFile, registry) {
  ...
  xml = xml.replace(/<w:drawing\b[\s\S]*?<\/w:drawing>/g, function (drawingXml) {
    ...
    var imageMarker = extractImageMarker(descr);
    if (!imageMarker) { return drawingXml; }
    var markerPath = imageMarker.path;
...
function extractOdtGenerateImageMarker (frameXml) {
  ...
  for (var i = 0; i < candidates.length; i++) {
    ...
    var parsed = extractImageMarker(text);
...
function scanXlsxDrawing (drawingFile, sheetFile, filesByName, registry) {
  ...
  drawingXml = drawingXml.replace(anchorRe, function (anchorXml, anchorType) {
    ...
    var descr = decodeXmlAttrValue(descrMatch[1]);
    var imageMarker = extractImageMarker(descr);
    if (!imageMarker) { return anchorXml; }
    var markerPath = imageMarker.path;
